...
public class DebuggerGG : MonoBehaviour
{
    ...
    private void Start()
    {
        if (AllowDebugging)
        {
            ...
            // StreamWriter fs = new StreamWriter(Application.streamingAssetsPath + "/DebugLog.txt", false);
            // fs.WriteLine("DeBugLog:");
            // fs.Close();
    ...
    public void LogText(string condition, string stackTrace, LogType type)
    {
        ...
        fs.WriteLine(log.time + "|" + log.message + "|" + type.ToString() + "|\n" + log.stackTrace);
        fs.Close();
    }
    ...
    private void LogHandler(string condition, string stackTrace, LogType type)
    {
        ...
        {
            log.type = "Fatal";
            if (_fatalLogCount < 9999) _fatalLogCount += 1;
        }
        else if (type == LogType.Exception || type == LogType.Error)
        {
            log.type = "Error";
            if (_errorLogCount < 9999) _errorLogCount += 1;
        }
        else if (type == LogType.Warning)
        {
            log.type = "Warning";
            if (_warningLogCount < 9999) _warningLogCount += 1;
        }
        else if (type == LogType.Log)
        {
            log.type = "Info";

            if (_infoLogCount < 9999) _infoLogCount += 1;
        }

        if (!DebugStop) _logInformations.Add(log);

        if (_logInformations.Count > 150) _logInformations.RemoveAt(0);
